  ...
  return (
    <Card variant="outlined">
        ...
        title={
          <div style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <Chip
              color="primary"
              size="small"
              label={_.padStart(_.size(user.coworkers), 2, 0)}
            />
          </div>
        }
        action={
      ...
      <CardContent style={{ padding: 0 }}>
        <List>
          {_.map(user.coworkers, coworker => (
            <UserItem key={coworker} user={coworker} />
          ))}
        </List>
      </CardContent>
    </Card>
...
MyColleagues.propTypes = {
  edit: PropTypes.func,
};

export default memo(MyColleagues);
